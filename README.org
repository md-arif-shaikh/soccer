...
* About the package
  ~Soccer~ is an [[https://www.gnu.org/software/emacs/][Emacs]] package for getting fixtures, results etc for soccer (football) matches inside emacs. Currently it works for only Premier League (England) but could be easily extended to add other leagues around the world. All data are fetched from [[https://www.scorespro.com/][scorespro]] website.
* Installation
